...
function Nav() {
  ...
  return (
    <>
      ...
      <div className="menu" onClick={() => setIsOpen((open) => !open)}>
...
function Main() {
  return (
    <main className="main">
      ...
      {manage.map((el) => (
        <ManageList title={el.title} text={el.text} num={el.num} key={el.num} />
      ))}
    </main>
  );
}

function ManageList({ title, num, text }) {
  return (
...
function TestimonialList({ avatar, author, testimonial }) {
  return (
    <div className="testimonial-card">
      <img src={avatar} alt={author} className="testimonial-card__photo" />
      <h3>{author}</h3>
      <blockquote className="testimonial-card__text">{testimonial}</blockquote>
    </div>
  );
}
